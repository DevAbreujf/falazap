...
export function DepartmentUsers({
  ...
  return <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button onClick={() => setIsAddingUser(true)} className="bg-emerald-400 hover:bg-emerald-300">
      ...
      <Table>
        ...
        <TableBody>
          {users.map(user => <TableRow key={user.id}>
              ...
              <TableCell>
                <UserActions onRemoveClick={() => setUserToRemove(user)} onChangeClick={() => setUserToChangeDepartment(user)} />
              </TableCell>
            </TableRow>)}
        </TableBody>
      </Table>
      ...
      <Dialog open={isAddingUser} onOpenChange={setIsAddingUser}>
        <DialogContent className="max-w-3xl">
          ...
          <div className="space-y-4">
            <div className="flex">
              <input type="text" placeholder="Buscar usuários..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full px-4 py-2 border rounded-md" />
            </div>
            <Table>
              <TableHeader>
                <TableRow>
              ...
              <TableBody>
                {filteredUsers.map(user => <TableRow key={user.id}>
                    ...
                    <TableCell>{user.email}</TableCell>
                    <TableCell>
                      <Button variant="outline" size="sm" onClick={() => handleAddUserToDepartment(user)}>
                        Adicionar ao Setor
                      </Button>
                    </TableCell>
                  </TableRow>)}
      ...
      <DepartmentChangeDialog open={!!userToChangeDepartment} onOpenChange={() => setUserToChangeDepartment(null)} departments={departments} currentDepartment={departmentName} onChangeDepartment={handleDepartmentChange} />
    </div>;
}