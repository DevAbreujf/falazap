...
export function PricingCard({ plan, index, onShowMore }: PricingCardProps) {
  ...
  return (
    ...
    >
      <div 
        className={`absolute inset-0 bg-gradient-to-b from-primary/10 via-primary/5 to-transparent rounded-xl blur-xl transition-all duration-500 ${
          isHovered ? 'opacity-100 scale-110' : 'opacity-0 scale-100'
        }`}
      />
      
      <div 
        className={`glass-card relative transition-all duration-300 hover:translate-y-[-4px] hover:shadow-2xl hover:shadow-primary/20 animate-fade-up flex flex-col max-w-sm mx-auto
      ...
      >
        ...
        <div className="text-center p-6 bg-gradient-to-b from-black/20 to-transparent">
        ...
        <div className="flex-grow px-6">
          <ul className="space-y-3 mb-6">
            {featuresToShow.map(([feature, value]) => (
              <li key={feature} className="flex items-center gap-2 group/item hover:bg-primary/5 p-1.5 rounded-lg transition-colors">
        ...
        <div className="p-6 pt-0 space-y-3">
          <Button 
            ...
            } rounded-lg font-semibold transition-all duration-300 hover:scale-[1.02]`}
